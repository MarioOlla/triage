...
public class AssegnaPazienteController {
	...
	public PazienteDTO assegnaPaziente(@PathVariable(required = true,name = "id") Long id, 
		...
		DottoreResponseDTO dottoreAggiornato = webClient.get().uri("/findByCodice/" + dottore.getCodiceDottore()).retrieve().bodyToMono(DottoreResponseDTO.class)
				.block();
		...
		.body(Mono.just(dottoreAggiornato), DottoreResponseDTO.class)
		.retrieve()
